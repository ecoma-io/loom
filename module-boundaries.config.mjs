...
export const boundarySuppressions = [
  ...
  },
  {
    ...
    messageId: "noRelativeOrAbsoluteImportsAcrossLibraries",
    reason:
      "the instrumented root specs (today: b2-shared-page, focus-not-obscured, keyboard, layout-responsive, page-sweep, reduced-motion) import the shared timing helper from ../playwright/timings. playwright/ is a Moon project but not an npm package: it has no package.json, so no published name exists to import instead — the same reach-as-data shape the tools row accepts, and the same reason wording applies. The helper is env-gated and contributes nothing to a production build; giving playwright/ a package.json and an export map so that six import statements could be spelled differently would be restructuring the repository to suit the spelling, so the reach is accepted and named instead.",
  },
  {
    path: "e2e/checks.ts",
    messageId: "noRelativeOrAbsoluteImportsAcrossLibraries",
    reason:
      "checks.ts is where the root sweep's per-page check bodies live, single-sourced so the production spec (page-sweep.e2e.ts) and the B2 bench (b2-shared-page.e2e.ts) assert the same ones — a bench variant cannot pass on weaker evidence than the gate it is meant to replace (ecoma-io/loom#381). Its runners time their phases with the same shared timing helper the specs import from ../playwright/timings. playwright/ is a Moon project but not an npm package: it has no package.json, so no published name exists to import instead — the same reach-as-data shape the tools row accepts, and the same reason wording applies. The helper is env-gated and contributes nothing to a production build; giving playwright/ a package.json and an export map so that one import statement could be spelled differently would be restructuring the repository to suit the spelling, so the reach is accepted and named instead.",
  },
  {
