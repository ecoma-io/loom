...
vi.mock("../../primitives/Spinner/Spinner.vue", async () => {
  const actual = await vi.importActual("../../primitives/Spinner/Spinner.vue");
...
vi.mock("../../primitives/Skeleton/Skeleton.vue", async () => {
  const actual = await vi.importActual("../../primitives/Skeleton/Skeleton.vue");
  const c = (actual as { default: { emits?: string[]; props?: Record<string, unknown> } }).default;
  return {
    default: {
      name: "Skeleton",
