...
| ------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------- |
| A component importing a facade **subpath** (`@ecoma-io/loom/theme`) | resolves the alias to `packages/loom/src/theme.ts`, the facade's own source, and judges `primitives → loom`, which the publishing boundary forbids — the same cycle a bare facade import is. The blind spot closed in 2E when the tsconfig path stopped landing on `packages/core`.  | Archkeep (`facade-subpath-import` mutation) + `check-architecture.ts` check 2          |
