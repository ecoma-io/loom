...
| TitleBar        | Pattern (borderline but defensible: window-chrome region) | `TitleBar.vue:12-15`                                                                             | Decided (ADR-001) — records the borderline-but-defensible Pattern judgement; lands in the renamed `packages/patterns/` during 2C | Phase 2 (with S4)          |
| DesktopAppShell | **Layout**                                                | meets the Layout definition on all four clause groups; deps exactly Pattern+Primitive+Foundation | Decided (ADR-001) — reclassify to Layout; moves to `packages/layouts/` in 2C                                                     | Phase 2: reclassify in 2C  |
| DashboardGrid   | **Composition** (decided)                                 | arrangement-only, container-driven, presentational (`DashboardGrid.vue:3-19`)                    | Decided (ADR-001) — Composition reclassifies to `packages/composition/` during 2C                                                | Phase 2 (reclassify in 2C) |
