...
not a shipped kind. The directory name is a legacy name whose migration is
decided (the `blocks → patterns` rename, Phase 2C); the rank order already
agrees with this model — both architecture readers place Pattern (the
`blocks` tier) **below Layout**, matching the hierarchy here.
...
| `packages/composition/*` (8 components)                                             | Composition                     | Aligned                               | Spatial arrangement only; layout-engine adapters live beside four of them (stack, inline, frame, center).                                                                                                                                                                                                                                                                              |
| `packages/blocks/*` (15 components)                                                 | Pattern                         | **Terminology + structural mismatch** | Semantically Pattern for the region intents (EmptyState, FormSection, RowActions…); DesktopAppShell instead meets this model's Layout definition and DashboardGrid reads as Composition (arrangement-only) — both flagged for the Phase 1 audit. Directory name says "blocks"; the rank is coherent with the model since Phase 2 (Pattern below Layout). Rename migrations land in 2C. |
