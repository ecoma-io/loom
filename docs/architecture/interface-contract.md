...
- No import-time browser code — gated: semgrep's leak rules catch leak-shaped
  ...
  `document`, `navigator`, `localStorage` or `matchMedia` outside a function,
  arrow or class body, `typeof` guards exempt — over the library surface the
  contract binds.
