...
| Accessibility             | Quality   | PARTIALLY_ENFORCED | `tools/check-a11y-evidence.ts` (in `pnpm lint` and its Verify step) holds every sidecar's declared role claim to the law in `packages/core/src/a11y-contract.ts` — the closed role vocabulary, the requirement matrix, per-requirement evidence cited in the tier that can witness it — and counts the named exceptions instead of failing on them (85 across 55 components at this writing, shrinking as evidence lands); depth witnessing — what a harness spec must exercise, not just that one exists — stays review-held until a depth mechanism lands (#308 item 4). Behind the gate: the zero-exclude sweeps + pinned contrast; per-component depth uneven                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| Semantic interaction      | Quality   | PARTIALLY_ENFORCED | Suites + per-component specs; every sidecar now declares its interaction class with evidence or a counted `because` exception (`check-interaction-evidence.ts`), the gap being the evidence the classes still owe                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
