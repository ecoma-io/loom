...
Evidence today: the structural half is gated. `tools/check-composition-conformance.ts`
(in `pnpm lint` and its Verify step) enumerates the composition family from the
tree — a directory read of `packages/composition/`, not a hand-maintained list —
and holds every member to the conformance evidence set (adapter, cases, spec,
coverage floor) or to a recorded exception whose reason, owner and removal
milestone are mandatory; an exception excuses absences only, and expires — a row
whose evidence has landed fails the gate itself. What stays review-held is the
composability half — props that describe interface intent, not product shape,
no contract only one application could fill — together with the business
boundary law ([constitution](./constitution.md) §4); the audit records the
enforcement status honestly.
...
Evidence today: the token rule is gated over the surface a literal read can
see — arbitrary bracket values on the style-bearing utilities, bare arbitrary
properties on theme-owned CSS properties, inline motion declarations and
template colour literals (hex, the CSS colour functions, a named colour
written as a style value) must be token-anchored or recorded in
`TOKEN_EXCEPTIONS` with the reason they stand (`tools/check-token-allowlist.ts`
in `pnpm lint` and CI, closing the "token-usage itself unlinted" gap). The
gate's limits are stated with it, not passed over: the named utility
spellings (`duration-100`, `z-50`, `opacity-50`, a default-palette
`bg-red-500`) carry the same decisions and pass unjudged until their own
migration lands, and the dynamic half of script-side styling stays
review-held. Behind the gate, `theme-core` tests pin contrast pairs
browserlessly, the axe and contrast suites re-check rendered output in light
and dark, and templates are held to both themes.
...
**The declared axis.** Which of those behaviours a component owes is a fact
of its interaction class, and the class is declared, not derived — the same
declared-not-derived doctrine the role claim follows, because reka-ui injects
interaction behaviour at runtime and no reader can derive the class from the
source with confidence. Every sidecar's `interaction` claim names one class of
a closed four-word vocabulary (`interactive`, `composite`, `container`,
`visual-only` — defined in `packages/core/src/a11y-contract.ts` beside the
role law), a `basis` naming the surface the class hangs on, and per duty
either evidence in the tier that can witness it or a counted exception with
its `because`: `keyboard-operate` (harness tier — a keyboard contract is a
browser fact, and the citation must itself bear a keyboard gesture) for every
class but `visual-only`; `state-report` (browserless tier) for the two
operating classes; `keyboard-inert` (browserless tier) for `visual-only`.
`tools/check-interaction-evidence.ts` holds every claim to that law and
counts the named exceptions instead of failing on them — the count shrinks
as evidence lands, and the gate's summary line reports it.

**New artifact duties:** the role's interaction story is written into specs,
not left to the default behaviour of the elements involved; the sidecar's
`interaction` claim lands with the component, naming the class and the honest
...
`pnpm lint` and CI) enforces the agreement mechanically across the whole
chain: the exports map, the vite `lib.entry` set, the emitted declarations,
the `./styles/*.css` sources, the docs' own `@ecoma-io/loom/<subpath>`
...
labels and layout-engine internals, the two barrel defaults the facade
withholds, the one internal sub-component — `TreeViewNode`, whose
`@internal-doc` record names no live marker today, the gate's designed inert
case, live the moment a docs page documents the sub-component's props with an
`<!-- @api TreeViewNode -->` marker) are
machine-parsed records in `packages/loom/src/index.ts`, each with its
reason, and a record that stops describing the code fails the gate like any
other drift.
...
| Accessibility             | Quality   | PARTIALLY_ENFORCED | `tools/check-a11y-evidence.ts` (in `pnpm lint` and its Verify step) holds every sidecar's declared role claim to the law in `packages/core/src/a11y-contract.ts` — the closed role vocabulary, the requirement matrix, per-requirement evidence cited in the tier that can witness it — and counts the named exceptions instead of failing on them, shrinking as the tranches tracked in #432 land — the count is deliberately not quoted here, because it would be stale by the next landing; the counts of record live in `tools/check-exception-registers.ts`, whose run prints the live-versus-record line; depth witnessing — what a harness spec must exercise, not just that one exists — stays review-held as a recorded decision: the interaction gate mechanized the one depth fact its duties need (a keypress in the cited spec), and generalizing that heuristic here would judge focus-traversal rules by keypress counts they do not measure. Behind the gate: the zero-exclude sweeps + pinned contrast; per-component depth uneven                                                                                                                                                                                                                         |     |
| Responsive                | Quality   | PARTIALLY_ENFORCED | `tools/check-responsive-evidence.ts` (in `pnpm lint` and its Verify step) holds every tiered component's declared `responsive` claim — behaviours spelled from the closed vocabulary, or a `contract: "none"` with its basis — to viewport-bearing evidence, with sweep citations read as tree facts, and counts the named exceptions instead of failing on them (4 across 4 components at this writing: 3 band-scale, 1 device-media); behind the gate: the site + template gates and the engine conformance route                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| Composition               | Quality   | PARTIALLY_ENFORCED | `tools/check-composition-conformance.ts` (in `pnpm lint` and the Verify job) enumerates the composition family from the tree and holds every member to the conformance evidence set — adapter, cases, spec, coverage floor — or to a recorded exception whose reason, owner and removal milestone are mandatory, which excuses absences only, and which fails the gate itself once its evidence lands (8 of 9 own the set; the one surviving row is scroll-reel's by-design declaration). Supersedes the audit row's four-composition census and its "a new composition can land with neither adapter nor cases" clause as of the audit date, with the ledger row to record the supersession at its next docs pass; the comparator's tolerance policy — the fixed pixel epsilon and the determinism quintet it presumes — is documented in the route it governs, `playwright/harness/conformance.ts`, and cited from there rather than restated here; the composability half of the contract — props that describe interface intent, not product shape — stays review-held                                                                                                                                                                                                  |
| Theming                   | Quality   | PARTIALLY_ENFORCED | `tools/check-token-allowlist.ts` (in `pnpm lint` + its Verify step): bracket arbitrary values on style-bearing utilities, bare arbitrary properties on theme-owned CSS properties, inline time literals and template colour literals are judged against the shapes in `packages/core/src/theme-contract.ts` — token-anchored or recorded in `TOKEN_EXCEPTIONS`; the blind spots are stated, not hidden: the named utility spellings (`duration-100`, `z-50`, `opacity-50`, a default-palette `bg-red-500`) and computed script-side values pass unjudged (review-held); behind it, contrast pins + dark gates                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| Semantic interaction      | Quality   | ENFORCED           | `tools/check-interaction-evidence.ts` (in `pnpm lint` and its Verify step) holds every sidecar's declared interaction class to the law in `packages/core/src/a11y-contract.ts` — the closed four-class vocabulary, the duty matrix, per-duty evidence cited in the tier that can witness it — and counts the named exceptions instead of failing on them; what the gate holds is the contract, not a census — every counted `keyboard-operate` row stands only until a component-owned harness spec answers it, and the count shrinks as the tranches tracked in #432 land. The count is deliberately not quoted here: a number in prose is right on the day it is written and wrong after the next landing, and the counts of record live in `tools/check-exception-registers.ts` — pinned two-way per register, each owner held to a live tracker — while `node --experimental-strip-types tools/check-exception-registers.ts` prints the live-versus-record line; the shrink discipline is enforced rather than asserted. What stays review-held: which keys a harness spec must exercise beyond pressing at least one — depth on this axis beyond the keypress tree fact is the `basis`'s prose (the role-axis statement of the same limit is in the Accessibility row) |     |
| Public API deliberateness | Quality   | ENFORCED           | `check-api-parity.ts` holds the whole chain to one surface — exports↔build↔declarations↔styles↔docs, every package barrel's parsed exports against the facade, the docs' `@api` markers, an identifier docs scan bounded by the register's declared generated-table classes, and the facade-manifest/tsconfig subpath mirror — with the deliberate withholds as machine-parsed records in `packages/loom/src/index.ts` whose drift fails the gate. The class bound is a declared suffix, not a per-identifier verification: a name exempted by a `@generated-docs` record is trusted to the mechanism that record states, which is the scan's named residual. Supersedes the audit's PARTIALLY_ENFORCED row as of its audit date, with the ledger row to record the supersession at its next docs pass (3F). What stays review-held is judgement, not agreement: whether a _new_ export should exist at all is a human call the gate can only force to be made deliberately, via the register it fails when the record is missing                                                                                                                                                                                                                                           |
