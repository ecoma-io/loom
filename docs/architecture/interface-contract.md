...
- One public npm package — `@ecoma-io/loom`; internal packages are private, declare no consumer exports — enforced: `tools/check-manifest-privacy.ts` asserts every `packages/<tier>/<name>/package.json` is `private` with no `publishConfig`, closing #238.
...
| One public package        | Invariant | ENFORCED           | `tools/check-manifest-privacy.ts` (in `pnpm lint`) asserts every `packages/<tier>/<name>/package.json` is `private` and carries no `publishConfig`; the root `@ecoma-io/loom` manifest is the only public package (#238 closed) |
