...
Evidence today: the token rule itself is gated — every style-bearing value a
component writes must be token-anchored or recorded in `TOKEN_EXCEPTIONS`
with the reason it stands (`tools/check-token-allowlist.ts` in `pnpm lint`
and CI, closing the "token-usage itself unlinted" gap; the dynamic half of
script-side styling is the gate's stated review-held limit) — and behind it
`theme-core` tests pin contrast pairs browserlessly, the axe and contrast
suites re-check rendered output in light and dark, and templates are held to
both themes.
...
| Theming                   | Quality   | ENFORCED           | `tools/check-token-allowlist.ts` (in `pnpm lint` + its Verify step): every style-bearing value token-anchored or recorded in `TOKEN_EXCEPTIONS`, computed script-side values review-held; behind it, contrast pins + dark gates                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
