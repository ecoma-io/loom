...
| Composition               | Quality   | PARTIALLY_ENFORCED | `tools/check-composition-conformance.ts` (in `pnpm lint` and the Verify job) enumerates the composition family from the tree and holds every member to the conformance evidence set — adapter, cases, spec, coverage floor — or to a recorded exception whose reason, owner and removal milestone are mandatory, which excuses absences only, and which fails the gate itself once its evidence lands (4 of 9 own the set; the 5 rows expire as Phase 4A lands the twins). Supersedes the audit row's four-composition census and its "a new composition can land with neither adapter nor cases" clause as of the audit date, with the ledger row to record the supersession at its next docs pass; the composability half of the contract — props that describe interface intent, not product shape — stays review-held                                                                                                                                                                                                         |
