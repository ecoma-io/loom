...
| Accessibility             | Quality   | PARTIALLY_ENFORCED | `tools/check-a11y-evidence.ts` (in `pnpm lint` and its Verify step) holds every sidecar's declared role claim to the law in `packages/core/src/a11y-contract.ts` — the closed role vocabulary, the requirement matrix, per-requirement evidence cited in the tier that can witness it — and counts the named exceptions instead of failing on them (86 across 56 components at this writing, shrinking as the tranches tracked in #432 land); depth witnessing — what a harness spec must exercise, not just that one exists — stays review-held as a recorded decision: the interaction gate mechanized the one depth fact its duties need (a keypress in the cited spec), and generalizing that heuristic here would judge focus-traversal rules by keypress counts they do not measure. Behind the gate: the zero-exclude sweeps + pinned contrast; per-component depth uneven                                                                                                                                                                                          |     |
...
| Semantic interaction      | Quality   | ENFORCED           | `tools/check-interaction-evidence.ts` (in `pnpm lint` and its Verify step) holds every sidecar's declared interaction class to the law in `packages/core/src/a11y-contract.ts` — the closed four-class vocabulary, the duty matrix, per-duty evidence cited in the tier that can witness it — and counts the named exceptions instead of failing on them (94 across 89 components at this writing, shrinking as the tranches tracked in #432 land). `check-exception-registers.ts` pins every register's count two-way and holds each register's owner to a live tracker, so the shrink discipline is enforced rather than asserted. What stays review-held: which keys a harness spec must exercise beyond pressing at least one — depth on this axis beyond the keypress tree fact is the `basis`'s prose (the role-axis statement of the same limit is in the Accessibility row)                                                                                                                                                                                        |     |
