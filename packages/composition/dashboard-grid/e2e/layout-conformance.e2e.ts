...
// DashboardGrid's live-oracle conformance spec (design D5): the engine tree
// the conformance route publishes must equal what the browser actually
// renders for the same props in the same viewport. Every navigation IS the
...
// between DashboardGrid.vue's classes and dashboardGridLayout's tree turns
// red.
//
...
// from Node: it carries DashboardGrid.vue with it, and Node cannot compile
// an SFC.
//
// DashboardGrid-specific: the case fixture widths are the spanned AREA
// widths at the case's container, and the container is the viewport width —
// so this suite's viewport must stay the width the cases were authored for.
// A vertical scrollbar would shrink the measured container and re-derive
// every track width; the cases' heights sum well under the viewport height
// to keep that from ever happening, and the engine side is safe by
// construction (it is fed the measured width, so its arithmetic always
...
const ROUTE = "dashboard-grid";
/** Every width a case is published at — the band plus the reflow boundary pair. */
const VIEWPORTS = [360, 702, 704, 800, 944] as const;
/** Tall enough that the case sections never stack past the fold — at 944 every case renders — because a vertical scrollbar would take 15px off every measured width: the fixture widths assume the container is the full viewport width. */
...
for (const vw of VIEWPORTS) {
  test.describe(`viewport ${vw.toString()}px`, () => {
    ...
    test("every DashboardGrid case published at this width renders exactly the geometry the engine computed", async ({
      page,
    }) => {
...
// 16px root are what make the rem-based tile floor and gap scale exact px
// arithmetic, and a profile change that breaks either must fail loudly, not
// flake softly. It carries its own viewport so the assertions are about the
// page, not about whatever the ambient default happened to be.
