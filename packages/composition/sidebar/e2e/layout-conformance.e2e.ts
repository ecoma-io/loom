...
// Sidebar's live-oracle conformance spec (design D5): the engine tree the
...
// between Sidebar.vue's inline styles and sidebarLayout's tree turns red.
//
// The case set covers the EXPANDED row only: the intrinsic collapse is
// flex-wrap line breaking, outside the engine's single-line subset, so the
// regime is guarded by sidebarCollapses and the coverage floor rather than
// modelled (SIDEBAR_UNMODELLED in ../src/layout.ts records what stays out
// and who owns it), and the browser side of the collapse stays with
// sidebar.e2e.ts.
//
...
// imported from Node: it carries Sidebar.vue with it, and Node cannot
// compile an SFC.
//
...
const ROUTE = "sidebar";
/** The bands Sidebar's gap scale distinguishes: below sm, and sm up. */
 ...
 * id "root" at or below the tree root, so synthetic wrappers (Center's
 * centering row) are descended through rather than compared.
 */
...
for (const vw of VIEWPORTS) {
  test.describe(`viewport ${vw.toString()}px`, () => {
    ...
    test("every Sidebar case published at this width renders exactly the geometry the engine computed", async ({
      page,
    }) => {
...
// not expect, pins the root font — the root sidebarLayout resolves rem
// sideWidths at — renders text-free fixtures, and measures the fixture
// container rather than trusting the viewport). DPR 1 and the 16px root are
// what make the rem-based basis and the percent resolution exact px
