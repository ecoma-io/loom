...
// Stack's live-oracle conformance spec (design D5): the engine tree the
...
// between Stack.vue's classes and stackLayout's tree turns red.
//
...
// imported from Node: it carries Stack.vue with it, and Node cannot compile
// an SFC.
//
...
const ROUTE = "stack";
/** The bands Stack's gap scale distinguishes: below sm, and sm up. */
...
for (const vw of VIEWPORTS) {
  test.describe(`viewport ${vw.toString()}px`, () => {
    ...
    test("every Stack case published at this width renders exactly the geometry the engine computed", async ({
      page,
    }) => {
...
// 16px root are what make the rem-based gap scale exact px arithmetic, and
