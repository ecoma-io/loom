...
// Inline's live-oracle conformance spec (design D5): the engine tree the
...
// between Inline.vue's classes and inlineLayout's tree turns red.
//
// Every case carries wrap: false explicitly — the adapter throws on the
// component's own default (wrap: true) because line collection is phase 2,
// so this spec only ever meets a single-line row. That makes it the spec
// that watches CSS's default flex-shrink actually run: the narrow-band gap
// cases overflow, and the browser's shrunk widths must equal the engine's
// scaled-shrink arithmetic to the pixel.
//
...
// Node: it carries Inline.vue with it, and Node cannot compile an SFC.
//
...
const ROUTE = "inline";
/** The bands Inline's gap scale distinguishes: below sm, and sm up. */
...
for (const vw of VIEWPORTS) {
  test.describe(`viewport ${vw.toString()}px`, () => {
    ...
    test("every Inline case published at this width renders exactly the geometry the engine computed", async ({
      page,
    }) => {
