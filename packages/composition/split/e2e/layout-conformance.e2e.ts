...
// Split's live-oracle conformance spec (design D5): the engine tree the
...
// between Split.vue's style objects and splitLayout's tree turns red.
//
// What is compared is the modelled subset: the one-line two-pane row. Split's
// intrinsic collapse is flex line collection, which the engine does not
// model — the adapter is total (a context-conditional throw would kill the
// whole page's report, since the route computes EVERY case at EVERY
// navigation), and the collapsed regime is guarded by staysOnOneLine and the
// coverage floor rather than modelled (SPLIT_UNMODELLED in ../src/layout.ts
// records what stays out and who owns it). The floor holds every case to the
// single line at every band for exactly that reason, which is also why no
// case carries the component's own 16rem default panel: in the narrow band
// the default Split is a wrapped Split. The collapse itself is held by the
// behavioural suite in split.e2e.ts, not here; Phase 4B's modelled-subset
// record owns it.
//
// The fixtures all land in the content area, because the route mounts only
// the default slot and Split's default slot IS the content area. The side
// panel is therefore the empty panel — and that is exactly what the
// comparison still pins about it: the basis width it never leaves, the
// stretch height the line gives it, and the position `side` puts it at.
//
...
// here instead of silently never running. The case module cannot be imported
// from Node: it carries Split.vue with it, and Node cannot compile an SFC.
//
// Why a known-divergence skip would be an annotated skip and not
// test.fixme: per-case granularity would require knowing the cases before
// the page loads, which the no-duplication rule above forbids; a runtime
// test.fixme() inside the per-case loop aborts the remaining cases with it.
// Split currently declares none — the coverage floor holds that open.

const ROUTE = "split";
/** The bands Split's gap scale distinguishes: below sm, and sm up. */
const VIEWPORTS = [360, 800] as const;
...
for (const vw of VIEWPORTS) {
  test.describe(`viewport ${vw.toString()}px`, () => {
    ...
    test("every Split case published at this width renders exactly the geometry the engine computed", async ({
      page,
    }) => {
...
// 16px root are what make the rem-based minSideWidth and the gap scale
// exact px arithmetic, and a profile change that breaks either must fail
// loudly, not flake softly. It carries its own viewport so the assertions
// are about the page, not about whatever the ambient default happened to be.
