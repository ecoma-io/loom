...
// Frame's live-oracle conformance spec (design D5): the engine tree the
...
// between Frame.vue's inline aspect-ratio and frameLayout's tree turns red.
//
// The ratio arithmetic is band-independent, so the case set carries two
// widths rather than three; what this spec watches that the others cannot
// is the overflow contract — a child taller or wider than the frame keeps
// its layout box in the browser's rect under the component's overflow:
// hidden (clipping is paint, not geometry), and the engine's recorded box
// must be that same unclipped rectangle.
//
...
// Node: it carries Frame.vue with it, and Node cannot compile an SFC.
//
...
const ROUTE = "frame";
/** The ratio arithmetic ignores the band; the two widths cover the default set. */
...
for (const vw of VIEWPORTS) {
  test.describe(`viewport ${vw.toString()}px`, () => {
    ...
    test("every Frame case published at this width renders exactly the geometry the engine computed", async ({
      page,
    }) => {
