...
describe("DatePicker calendar", () => {
  ...
  it("draws an out-of-bounds day in colour and a strike rather than fading its number", async () => {
    ...
    // Guarded off the selection so the two colours can never end up ordering
    // against each other: the chosen day keeps its own text colour whatever
    // else is true of it.
    expect(
      getDay(now.toString()).classList.contains("data-[selected]:text-primary-foreground"),
    ).toBe(true);
  });
