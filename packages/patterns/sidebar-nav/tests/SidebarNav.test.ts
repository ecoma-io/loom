...
import SidebarNav, { type SidebarNavItem, type SidebarNavSection } from "../src/SidebarNav.vue";
...
vi.mock("@ecoma-io/loom-tooltip", async () => {
  const actual = await vi.importActual("@ecoma-io/loom-tooltip");
...
vi.mock("@ecoma-io/loom-badge", async () => {
  const actual = await vi.importActual("@ecoma-io/loom-badge");
  const c = (actual as { default: { props?: Record<string, unknown> } }).default;
  return {
    default: {
      name: "Badge",
...
vi.mock("@ecoma-io/loom-separator", async () => {
  const actual = await vi.importActual("@ecoma-io/loom-separator");
  const c = (actual as { default: { props?: Record<string, unknown> } }).default;
  return {
    default: {
      name: "Separator",
